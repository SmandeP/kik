...
#include "ui_zchncontroldialog.h"
...
std::list<std::string> ZChnControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZChnControlDialog::listMints;

ZChnControlDialog::ZChnControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZChnControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZChnControlDialog::~ZChnControlDialog()
{
    delete ui;
}

void ZChnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZChnControlDialog::updateList()
{
...
void ZChnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZChnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZChn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZChnControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZChnControlDialog::GetSelectedMints()
{
...
void ZChnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
