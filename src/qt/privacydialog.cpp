                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCHN ought to be enough for anybody." - Bill Gates, 2017
    ui->zCHNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCHNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCHN->setEnabled(false);
        ui->pushButtonMintzCHN->setToolTip(tr("zCHN is currently disabled due to maintenance."));

        ui->pushButtonSpendzCHN->setEnabled(false);
        ui->pushButtonSpendzCHN->setToolTip(tr("zCHN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCHNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCHN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCHN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCHN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCHN in ") +
...
void PrivacyDialog::on_pushButtonSpendzCHN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCHN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCHN
        sendzCHN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCHN
    sendzCHN();
}

void PrivacyDialog::on_pushButtonZChnControl_clicked()
{
    ZChnControlDialog* zChnControl = new ZChnControlDialog(this);
    zChnControl->setModel(walletModel);
    zChnControl->exec();
}

void PrivacyDialog::setZChnControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCHNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCHN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Chronos Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCHNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCHN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CHN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCHN</b>";
    ...
    // use mints from zChn selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZChnControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZChnControlDialog::GetSelectedMints();
    }
    ...
    // Clear zchn selector in case it was used
    ZChnControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zChn Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Chn, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zChn Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCHN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCHN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCHN "));
    ui->labelzCHNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCHNSyncStatus->setVisible(fShow);
}
